...
The first annotated screenshot is of the upper section of the About Us page. To maintain consistency, the page uses the same orange / cream background as the rest of the site. A full width hero banner displays a dog image with a parallax style effect achieved with background-attachment: fixed. The main "About Us" title is a bold white H1 with a text shadow and a size of 80px, remaining readable against the photograph.

Below the banner sits the "Who We Are" section inside a container with a border radius of 10px and a light box shadow. The section heading uses an H1 with the primary orange underline, supporting a familiar layout consistent with Jakob's Law. A German Shepherd image (Rusty) is also shown with a 20px border radius and object fit as cover. This is in a flex layout so the image and text adapt on different screen sizes.

The second annotated screenshot includes the "Meet Our Team" section as you scroll further down the page. It repeats the same orange underline under heading style and cream background for visual consistency. Three team profiles are displayed in circular images with a magnify effect when hovered over. Each members name appears in H3 with a font weight of 600 and a 16px text beneath describing their role.

JavaScript was also used for this page to create a fade in animation effect when the page loads and when sections scroll into view. This makes the page dynamic and matches the modern design of current websites.
...
This is the annotated Browser Page where users can search and browse adoptable pets. Each pet card is linked to the pet_selection page with updating details depending on the card picked. The Browser Page opens with a centred black H1 heading reading "Adoptable Pets" in bold and 54px size on the same orange / cream background seen on every other page. The layout splits into a left filter panel and a right content area grouped by pet type.

The page also has a filter sidebar and an included search bar, which allows you to search by brand or name. Type buttons, location checkboxes and gender filters are also there which once set can easily be reset with the "Reset All" control in the top right. JavaScript applies these filters in real time and hides empty sections, showing a "no results" message when nothing matches. Each pet card sits in a white section box with a 10px border-radius and light shadow for contrast. These boxes display a photo, name and breed of the pet. Cards use data attributes and unique IDs to update info on pet_selected, linking to the JSON database used on the detail page. Pet images are sourced from Unsplash for consistent, copyright-free visuals across the browser and pet selected pages.
...
This is the annotated contact us page which uses the standard orange / cream background and again a centred black H1 title reading "Contact Us". A 30px H3 subtitle is also beneath that explaining that users can send questions or feedback via the form below. The main content is a two column contact box: a white left panel for the enquiry form and an orange right panel for centre details.

The "Send Your Request" form includes labelled fields for name, email, phone, subject and message, with HTML5 validation and placeholder example text. The oval shaped orange "Submit" button sends data to `php/data.php` via JavaScript `fetch`. On success the form is hidden and a confirmation message is shown, with a "Send Another Message" button to reset. The "Reach Us" panel lists email, phone and address in white text with the primary orange colour as the background. A embedded Google Maps iframe is also there shown the Bedford Park campus location to help users find the adoption centre. The signature orange and split layout follow the same visual theme as other pages (Jakob's Law), while the shared header and nav bar keep movement between pages straightforward.
...
The back-end communication for the Adopt It website was mainly implemented using JavaScript and JSON to create dynamic interactions between pages and user inputs. One of the key implementations was on the pet selected page, where a JSON file was used to store pet information such as names, breeds, descriptions, medical notes, and images separately for all pets. When a user selected a pet from the browser page, JavaScript retrieved the corresponding data from the JSON file structure and dynamically updated the page content and images. This allowed multiple pet information and images to be displayed using a single html page design rather than creating separate HTML pages for every pet.
...
PHP back end in `php/data.php` was called from the front end using JavaScripts fetch with an action parameter sent via POST and GET for read operations. The contact, adoption and sign in pages forms when submitted use the actions save_contact, save_adoption and save_use. PHP is also used for basic input sanitation in `php/sanitize.php`. Each field is sanitised through this file before being appended to the matching JSON file. The server then responds with JSON (e.g., "success: true, "message"...) and the client uses this to show a confirmation message or an error alert if it fails. get_pets and get_users read operations are also supported for retrieving stored data from `pets.json` and `users.json` when required.
...
| Text       | `#000000`, `#2c3e50` , `#888888` | Body text is black as it works best against the light coloured backgrounds used. Navigation links used `#2c3e50`, following similar style to hyper links. Secondary text such as breed labels used grey for contrast against white |
...
Over the development process of our pet adoption website called "Adopt It", I learnt and gained valuable experience when turning UX design concepts from Assignment 1 into working web pages. In the earlier assignment, I contributed to personas, wireframes, user flows, and a sitemap for the browser and admin areas. Then for Assignment 2, I applied that planning to build the About Us, Browser Page and Contact Us pages using HTML, CSS, and JavaScript, strengthening my understanding of how early design decisions affect implementation and usability.

My task was developing the browser, contact and about us pages. On the browser page, I implemented a search and filter section using JavaScript so users could narrow down their listing sorting by type, gender and location. Each pets card then linked to the pet selected page via URL parameters and details updated with the pets.json file I created. The contact and about us pages included form validation done with JavaScript, confirmation done with PHP, and layout styling done with CSS. A significant challenge was keeping the site visually consistent across other group members pages. I worked on shared elements such as the header and footer scripts, where we ran into some git conflict, that had to be resolved. Eventually we got a system that updates the sign-up button to a profile link when a user is logged in and is true to global styling. Pages which has small inconsistencies in styling I also updated to ensure the style guide summary was accurate.

To support successful deployment, I helped organise our folder structure by team member, fixed broken paths, and coordinate updates such as the JSON pet database and header behaviour to avoid merge conflicts. I also aimed for clear navigation and semantic HTML, so pages remained readable and accessible across different screen sizes. Debugging small issues such as inactive links due to path errors, margin errors and pet not found handling greatly improved my problem solving skills and through consistent communication with group members, we could successfully create a website that deepened my knowledge of teamwork, front-end development and how to create using a user centred design approach.
...
Apart from designing the pages, I managed to develop CSS styles for the whole web application and create header, footer, and navigation bar for the website. At the same time, thanks to the changes made by my teammates to the pages, the issue of redundancy could be avoided while creating the web application. Nevertheless, there were still some challenges I had to deal with like the website contained lots of similarities in terms of color, fonts, and spaces used. Therefore, it was important to stick to the existing design and making sure that the common parts were all the same.

To achieve a successful webpage, I think that communication within the team and planning accurately before starting is crucial and I think that assignment 1 – UX design report helped us a lot where we planned out on how our web page is going to look with the help of the wireframes and storyboards that we designed. Also, we found it difficult at the start on how to think of working together on the same set of pages at once and then we figured it out through GitHub where it helped us to work on our individual pages separately but at the same time. Overall, during this project I gained a lot of knowledge regarding web development and improved my coding practices as well.
